...
fs.writeFileSync(
  "/home/pi/smart_house/smart_house_fe/public/ip.json",
  JSON.stringify({
    address,
  })
);
...
app.post("/log", (req, res) => {
  ...
  db.query(
    "SELECT email FROM users WHERE email = ? AND psw = ?;",
    [email, hash],
    (err, response) => {
      if (err) {
        console.log(err);
...
app.post("/alarm", (req, res) => {
  db.query(
    "SELECT * FROM alarm ORDER BY ID_alarm DESC LIMIT 3",
    (err, response) => {
      if (err) {
        console.log(err);
        return;
...
const interval = setInterval(() => {
  if (listPin.readSync() === 1 && isInserted === false) {
    ...
    db.query(
      "INSERT INTO alarm(day,hh,mm,ss) VALUES(?,?,?,?)",
      [now, hh, mm, ss],
      (err, res) => {
        if (err) {
          console.log(err);
